...
$.jQchart={
	version  : '0.02-beta-2.0b',
	updateMemo : 'v0.02-beta-2.0 are going to support "bar type"',
	charset  : 'utf-8',
	...
	function drawChart(_option){
		...
		jQuery.extend( $.fn.jQchart,{
			...
			init :	function (op){ 
				...
				this.op.paddingB=(!this.op.labelX)?'8':this.op.paddingB, //Xラベル無時のサイズ補正
			...
			setXaxis : function(op){
				...
				for (var i = 0; i <= this.op.grid.row; i++) { 
					this.drawAxis(this.op);
					this.util.setNextY(this.op); 
				}
				
				this.op.setXaxis=true;
			},
			...
			drawBar : function (op,it,ofs){
				...
				for(var i=0;i<dlen;i++){
					...
					x=op.paddingL+ this.op.barWidth*(i+1)
				}
				
				function drawRect(i,j,ofs){

					y = op.paddingT+op.chartHeight
								 -op.rows[j]*op.chartHeight/op.dataYmax ; //alert("--y"+y+"--x"+x+"--i"+i+"--j"+j+"--"+op.data[i][j])
