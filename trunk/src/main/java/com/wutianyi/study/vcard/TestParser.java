...
public class TestParser {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < vcards.size(); i++) {
			...
			System.err.println(nameFeature.getFamilyName() + " "
					+ nameFeature.getGivenName() + " "
					+ nameFeature.getAdditionalNames() + " "
					+ nameFeature.getHonorificPrefixes()
					+ nameFeature.getHonorificSuffixes());
			PhotoType photo = new PhotoType();

			photo.setPhoto(FileUtils.readFileToByteArray(new File(
					"file/vcard/admin-bar-sprite.png")));
			...
			if (writer.hasErrors()) {
				...
				for (int j = 0; j < errors.size(); j++) {
					System.out.println(StringUtil.formatException(errors.get(j)
							.getError()));
				}
