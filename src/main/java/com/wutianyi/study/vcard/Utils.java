...
public class Utils {
	...
	public static void buildValues(Header header, VcardWrapper vcard,
		...
		switch (vCardType) {
		case XTENDED: {
			WrapperInterface[] extendedFeatures = vcard
					.getValues(VCardType.XTENDED);
			if (null != extendedFeatures) {
				for (int index = 0; index < extendedFeatures.length
						&& i < count; index++) {
					if (StringUtils.equals(header.getvKey(),
							extendedFeatures[index].getType())) {
						buildValue(header, extendedFeatures[index].getValue(),
								builder);
						if (i != count - 1) {
							builder.append(',');
						}
				...
				for (int index = 0; index < features.length && i < count; index++) {
					...
					if (null != parameters && parameters.length > 0) {
						if (features[index].containParameter(parameters)) {
							buildValue(header, features[index].getValue(),
									builder);
							i++;
							if (i < count) {
								builder.append(',');
						...
						if (features[index].getParameterSize() == 0) {
							buildValue(header, features[index].getValue(),
									builder);
							i++;
							if (i < count) {
	...
	private static void appendEmpty(Header header, int count,
			StringBuilder builder) {
	...
	public static ReportDO createReportDO(List<VcardWrapper> vcards,
			ReportDO report) {
				...
				case XTENDED: {
					...
					if (null != wrappers) {
						for (WrapperInterface wrapper : wrappers) {
							if (StringUtils.equals(header.getdKey(), wrapper
									.getType())) {
								++count;
							}
						}
